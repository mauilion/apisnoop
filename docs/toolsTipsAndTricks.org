...
- Get Read Only link for tmate session: =$ tmate display -p '#{tmate_ssh_ro}'=
...
** Flip between emacsclient Tilt, and Bachb 1 ..and then C-b and then 0=
...
- To commit: =c c= , =i= enter a discription of the commit, =Esc=, =, ,= to commit. To push the commit =P u= to create and push to the branch. 
- Further reading: [READ](https://gitlab.ii.coop/ii/org/blob/master/training/spacemacs-workflow/spacemacs-workflow.org)
* Toggle numbering for function blocks    
- SPC n f Zoom on only function with numbering for the funtion only
- SPC n w Zoom out to numbering for all documentation
- SCP t n Toggle between number on and off
- SPC t r Relative numbers, change back with SPC t n
* Mattermost code blocks
-Putting code blocks in Mattermost chat
``` Followed by the code block, closing again with ```
* Making presentations out of Org files with [reveal.js](reveal.js.com)
- Start with the template file. ie. github.com/cncf/apisnoop/org/roadmap.org
- Create the file content as per the example. the Footnotes contain the code to run the presentaion.
- Once the file content is ready to present type: ,e e v b (Ctrl n allow to scroll down the screen to see the rest of the menu)
- Changes can be made to the footnotes  by changing between REVEAL and NOREVEAL and ammending Options
