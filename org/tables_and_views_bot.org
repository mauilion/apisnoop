    ...
    if part in current_level:
      ...
      if len(variable_levels) == 1:
        try:
          ...
          plpy.warning("variable_levels: " + ','.join(variable_levels))
          raise err
        try:
          # raise "Testing to make sure it does the thing"
          current_level = current_level[next_level] # variable part is final part
        except Exception as err:
          plpy.warning("next_level was:" + next_level) 
          plpy.warning("current_level keys:" + ','.join(current_level.keys()))
          raise err
    else:
