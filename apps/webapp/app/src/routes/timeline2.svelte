<script>
 ...
 import originalStats from '../data/release_stats.json';
 import originalTimeline from '../data/timeline.json';
 ...
 const V1_12_RELEASE = dayjs("2018-09-26");
 const stats = originalStats
   .filter(stat => dayjs(stat.date).isAfter(V1_12_RELEASE));

 const dates = stats
   .map(s => s.date)

 const timeline = originalTimeline
   .filter(event => dayjs(event.date).isAfter(V1_12_RELEASE) && event.type !== 'release');
 let width = 900;
 let height = 400;

 const releases = originalTimeline
   .filter(event => dayjs(event.date).isAfter(V1_12_RELEASE) && event.type === 'release')
       .map(event => ({release: event.event.split(' ')[0],
                      ...event
                      }))
...
<div class="chart" bind:clientWidth={width} bind:clientHeight={height}>
 ...
 <text
   ...
   {currentStats.testedText}
    ...
    <g class="releases">
      {#each releases as r}
        <g class="release release-{r.event}" transform="translate({xScale(dayjs(r.date))},{yScale(0) - 2})">
          <text>{r.release}</text>
        </g>
      {/each}
    </g>
