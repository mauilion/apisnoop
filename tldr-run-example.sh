...
# export TPROXY_POD=$(./list-pod-nodes.sh | grep "^tproxy-tproxy-[a-z0-9]{5} .\+$APP_NODE" | awk '{print $1}')
export TPROXY_POD=$(./list-pod-nodes.sh | grep $APP_NODE | grep tproxy | grep -v tproxy-initial )
